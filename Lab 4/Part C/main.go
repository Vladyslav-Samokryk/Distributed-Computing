...
func changePrice(readWriteLock chan int){
	...
	for  {
		...
		fmt.Println("Price before changing", routes[random].destination, "=", routes[random].price,"$")
		routes[random].price = float64(rand.Intn(100))
		fmt.Println("Price after changing", routes[random].destination, "=", routes[random].price,"$")
		fmt.Println("")
		i++
		time.Sleep(time.Second)
		readWriteLock <- 1
	...
	for  {
		...
		if count == 2{
			for i:=0;i<len(routes);i++{
				...
				if substrA == (cityA+","){
					if substrB == (cityB+"."){
						fmt.Println("Price of route", routes[i].destination, "=", routes[i].price)
						found = true
						break
					}
				}
...
func init()  {
	...
	routes = append(routes, Route{"from: A, to: B.", 13})
	routes = append(routes, Route{"from: B, to: C.", 15})
	routes = append(routes, Route{"from: C, to: D.", 20})
	routes = append(routes, Route{"from: D, to: E.", 25})
}
